...
function VTerm(container_id, context) {
	...
	t.inputdiv = addEl(
		addEl(t.container, "div", "input-container"),
		"div",
		"input-div",
	);
	...
	t.btn_clear = addBtn(k, "key", "✗", "Ctrl-U", function (e) {
		t.set_line("");
		t.show_suggestions(_getCommands(this.context).map(addspace));
	});
	t.btn_tab = addBtn(k, "key", "↹", "Tab", function (e) {
		t.make_suggestions();
	});
	t.btn_enter = addBtn(k, "key", "↵", "Enter", function (e) {
		t.enterKey();
	});
	t.msg_idx = 0;
	t.soundbank = {};
...
VTerm.prototype = {
	...
	make_suggestions: function (tabidx, autocomplete) {
		...
		if (args.length > 0) {
			...
			function trymatch(potential, tocomplete) {
				var tocompleterx = new RegExp(
					"^" + t.complete_opts.fuzzy(tocomplete),
					t.complete_opts.case,
				);
			...
			if (tocomplete && idx > 0) {
				...
				if (_hasRightForCommand(args[0], user.groups)) {
					// propose argument
					match = _completeArgs(args, idx, tocomplete, t.context, trymatch);
				} else {
					// propose command completion
	...
	show_suggestions: function (list, highlights) {
		...
		this.suggestions.innerHTML =
			'<div class="visually-hidden">' + _("Suggestions") + "</div>";
	...
	show_suggestion: function (txt, hlcls) {
		...
		addBtn(
			t.suggestions,
			hlcls,
			txt.replace(/(#[^#]+#)/g, '<i class="hashtag"> $1 </i>'),
			txt,
			function (e) {
				t.input.value += txt;
				//      var l=t.get_line();
						...
						t.answer_input.onkeyup = function (e) {
							if (e.key === "Enter") {
								if (e.ctrlKey || !args.multiline) {
									...
									var bef = t.answer_input.value.substr(0, strt),
										aft = t.answer_input.value.substr(strt);
	...
	auto_shuffle_input_msg: function (
		msg,
		fromcomplexicity,
		tocomplexicity,
		stepcomplexity,
	...
	shuffle_input: function (minsize, maxsize) {
		var l =
			minsize + (maxsize ? Math.round(Math.random() * (maxsize - minsize)) : 0);
