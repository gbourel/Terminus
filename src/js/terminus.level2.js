	...
	newRoom("clearing", "loc_clearing.gif", { writable: true, executable: false })
		...
		.addStates({
			HouseMade: function (re) {
				if (re) {
					$clearing.addPath(newRoom("house"));
				}
				$clearing
...
$clearing.addPath(newRoom("ominouspath", "loc_path.gif", { writable: true }));
$ominouspath
...
// SLIDE
$trollcave.addPath(
	newRoom("slide", null, { executable: false }).setCmdText(
		"cd",
		_("room_slide_cd"),
	),
);
...
var $bigfiles = $morekernel.newItemBatch("bigfile", [
	"L",
	"M",
	"Q",
	"R",
...
$bigfiles[Math.floor(Math.random() * 9)].setCmdText("grep", "password = IHTFP");
